...
import com.jstorm.common.CDRException;
...
public class StandardFtpHelper extends FtpHelper {
	private static final Logger LOG = LoggerBuilder.getLogger(StandardFtpHelper.class);
...
//			ftpClient.setControlEncoding(fileEncoding);
	...
	public boolean isFileExist(String filePath,List<String> fileSuffix) {
		...
		try {
			FTPFile[] ftpFiles = ftpClient.listFiles(new String(filePath.getBytes(),FTP.DEFAULT_CONTROL_ENCODING), null);
...
//			} else
			...
			try {
				...
				long currentLastModifyTime = lastModifyTime;

				FTPFile[] fs = ftpClient.listFiles(new String(directoryPath.getBytes(),FTP.DEFAULT_CONTROL_ENCODING),new UserFtpFileFilter(fileSuffix, lastModifyTime));
				LOG.info("ftpFile list...{}", fs);
				for (FTPFile ff : fs) {
...
//					} else
					if (ff.isFile()) {
						...
						if(ff.getTimestamp().getTime().getTime() > currentLastModifyTime){
							currentLastModifyTime = ff.getTimestamp().getTime().getTime();
						}
				...
				// 更新缓存中的下载文件时间
				Element newLastModifyTimeElem = new Element("fileTime", currentLastModifyTime);
				CacheUtils.getDownloadCache().put(newLastModifyTimeElem);
